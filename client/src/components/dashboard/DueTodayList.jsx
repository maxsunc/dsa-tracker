export default function DueTodayList({ problems, onReview }) {
  return (
    <div>
      ...
      <div className="bg-white dark:bg-zinc-800 rounded-xl border border-purple-200 dark:border-purple-500/30 divide-y divide-gray-100 dark:divide-zinc-700">
        {problems.map((up) => (
          <div key={up.id} className="flex items-center justify-between p-4">
            <div>
              ...
              <div className="flex gap-2 mt-1">
                ...
                <span className="text-xs text-gray-400 dark:text-zinc-500">·</span>
                <span className="text-xs text-gray-500 dark:text-zinc-400">{up.problem.pattern}</span>
              </div>
            </div>
            <button
              onClick={() => onReview(up)}
